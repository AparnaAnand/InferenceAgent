    ...
    if ind!=2:
        for eachRule in match:
            ...
            if ind==1:
                ...
                s2=UNIFY(eachRule['Conclusion']['Args'],goal['Args'],subList)
    ...
    for x in BW_OR(Rules,Facts,query,{}):
        ...
        if x is not False:
            ...
            if query['Pred'] not in Facts:
                Facts[query['Pred']]=[]
            Facts[query['Pred']].append({'Pred':query['Pred'],'Args':query['Args']})
...
for each in rules:
    ...
    if Cpred not in Rules:
        Rules[Cpred]=[]
    Rules[Cpred].append({'Conclusion':{'Pred':Cpred,'Args':Cargs},'Premises':[]})
